...
async function readBalance(credentials) {
  ...
  const infos =
    payload && Array.isArray(payload.balance_infos)
      ? payload.balance_infos
      : [];
  const info = infos[0];
  if (info === void 0 || info === null) return { ok: false };
  const balance =
    typeof info.total_balance === 'string' ? Number(info.total_balance) : NaN;
...
function installSettings(ctx) {
  ctx.inject(['settings'], (sctx) => {
    sctx.settings.register(SETTINGS_NAMESPACE, PeakSettingsSchema);
  });
}
