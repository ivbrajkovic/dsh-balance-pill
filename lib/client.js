...
//
// The pill view registers twice: into `conversation.session.header.utilities`
// (additive, replaceRisk: none) for in-session display inside the header flex
// row, and into the frame-wide `shell.overlay` seat (pinned top-right) shown
// only while no session header is showing its own pill, so the pill is always
// visible without covering header controls. The two seats are mutually
// exclusive views over one shared balance store (`createBalanceStore`) that
// owns the single poll of the Host route — first fetch on the first mounted
// view, then every 60s, plus on click — so the API is polled once per minute
// no matter which seat is mounted. The store only ever holds
// { ok, balance, currency } or { ok: false } — no key material, no error
// detail.
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Single page-lifetime balance poll shared by every mounted pill seat.
		 * The fetch loop starts lazily on the first subscriber and keeps a 60s
		 * cadence from then on: exactly one pill is always mounted (the two
		 * seats are mutually exclusive), so the loop never idles meaningfully,
		 * and seat switches remount a view over warm state instead of
		 * triggering a fresh request. `refresh` is the click handler and
		 * coalesces with an in-flight poll.
		 */
		 ...
		 * while loading or after any failure. Clicking refreshes at once. All
		 * data lives in the shared stores: balance from the single poll
		 * (`balance`), peak-hours config from the settings store (`peak`) — the
		 * view owns no fetch loop of its own, so two mounted seats would still
		 * poll once. It tints the button red during a peak range and green
		 * otherwise.
		 * @param peak - the peak-config snapshot store.
		 * @param balance - the shared balance store (single poll + click refresh).
		 * @returns the pill component.
		 */
		function makeBalancePill(peak, balance) {
			return function BalancePill() {
				const state = react.useSyncExternalStore(balance.store.subscribe, balance.store.getSnapshot);
				const config = react.useSyncExternalStore(peak.store.subscribe, peak.store.getSnapshot);
				const [peakTint, setPeakTint] = react.useState({ running: false, timeZone: "UTC" });
				...
				let content;
				if (state.kind === "ok") {
					const peakLabel = peakTint.running ? "peak hours" : "off-peak";
				...
				return react.createElement(
					"button",
					{
						...
						title,
						className: "dsh-balance-pill" + (peakTint.running ? " dsh-balance-pill--peak" : " dsh-balance-pill--offpeak"),
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(CSS_TAG) + "]") === null) {
			...
			tag.textContent = [
				...
				// In the frame-wide shell.overlay seat (no session open) the pill is
				// pinned to the top-right corner; inside the session header it stays
				// a plain flex item and never covers header controls.
				"[data-shell-overlay] .dsh-balance-pill{position:absolute;top:12px;right:12px}",
		...
		/** Required services: the slot registry the always-visible seat lives on. */
		const inject = ["slots"];

		/**
		 * Client plugin body: contribute the pill to the session-header utilities
		 * seat (inside the header flex row) and, as a frame-wide fallback, to
		 * `shell.overlay` (pinned top-right) shown only while no session header
		 * exists — no current session, or a still-blank one — so the pill is
		 * always visible yet never covers header controls. Also registers the
		 * peak-hours settings card into the Plugins configuration page. The pill
		 * reads the same settings store the card writes, so a change retints it.
		 * @param ctx - client root context.
		 */
		function apply(ctx) {
			const peak = createPeakConfigStore();
			const balance = createBalanceStore();
			const BalancePill = makeBalancePill(peak, balance);
			// In-session placement: the pill rides the header's utilities seat as a
			// plain flex item, so it never overlaps the header's own controls.
			...
			// Always-visible fallback: the same pill view pinned to the top-right
			// corner of the frame, rendered only while the header is not showing
			// its own pill. The session open in the main view is the list row
			// retained by `mainView` (SessionListState carries no "current" field —
			// this is the same projection the frame's document title reads). A
			// blank main-view session keeps the header chrome hidden, so the
			// overlay view stays on for it too.
			const OverlayBalancePill = function OverlayBalancePill(props) {
				const headerPresent = props.useSessions((s) => {
					const current = Object.values(s.byId).find(
						(session) => (session.retainedBy.mainView ?? 0) > 0
					);
					return current !== undefined && current.blank !== true;
				});
				if (headerPresent) return null;
				return react.createElement(BalancePill);
			};
			ctx.slots.inject("shell.overlay", () =>
				ctx.slots.register(
					{ name: "shell.overlay", id: "balance-pill", order: 10 },
					OverlayBalancePill
				)
			);
