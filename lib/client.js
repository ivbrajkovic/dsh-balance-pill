...
//
// The pill registers twice: into `conversation.session.header.utilities`
...
// only while no session header exists, so the pill is always visible without
// covering header controls. It polls the Host route on mount, every 60s, and
// on click. It only ever receives { ok, balance, currency } or { ok: false } —
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// Always-visible fallback: the same pill pinned to the top-right corner
			// of the frame, rendered only while no session header exists (no
			// current session, or a still-blank one). Once a real session header
			// mounts, this entry renders null so only the header pill shows.
			const OverlayBalancePill = function OverlayBalancePill(props) {
				const useSessions = props.useSessions;
				const headerAbsent = useSessions((s) => {
					const current = s.current;
					return current === undefined || s.byId[current]?.blank === true;
				});
				if (!headerAbsent) return null;
